function [R,channelGaindB, UEpositions, BSpositions] = functionExampleSetup(L,K,M,ASDdeg, antennaType)
...
sigma_sf = 0; %neglect shadow fading
...
%BSpositionsWrapped = BSpositions ; % IF WE DON'T WANT A WRAP AROUND TOPOLOGY OTHERWISE COMMENT THIS
...
if DEBUG
    figure;
    scatterplot(BSpositionsWrapped(:)); hold on;
    xticks(0:squareLength/nbrBSsPerDim:squareLength)
    yticks(0:squareLength/nbrBSsPerDim:squareLength)
    grid on;
end
...
for l = 1:L
    ...
    if DEBUG
        plot(real(UEpositions(:,l)), imag(UEpositions(:,l)),'*r')
    end
    %Go through all BSs
    for j = 1:L
        ...
        for k = 1:K
            ...
            for spr = 1:length(ASDdeg)
                if strcmp(antennaType, 'ULA')
                    R(:,:,k,l,j,spr) = functionRlocalscattering(M,angleBSj,ASDdeg(spr),antennaSpacing); %uniform linear array
                elseif strcmp(antennaType, 'UCA')
                    R(:,:,k,l,j,spr) = functionRlocalscatteringApproxUCA(M,angleBSj,ASDdeg(spr)); %circular array
                end
            end
        end
