function sectionIII_fig2_RCUs_exact(n, np, rho_db, b, angle2, Mlist, nbrOfRealizations, ESTIMATOR, COMBINER, PILOT_CONTAMINATION, UNCORRELATED, NO_OF_UEs, BS_CSI, UE_CSI)
...
% Generates the exact RCUs points in Fig. 2 for both UL and DL. 
% 
% INPUTS:
% n = blocklength
...
parfor j = 1:length(angle2)
    ...
    % Computation of the UL average error probability:
    i_s = @(s) info_density(s,g_list,g_hat_list,sigma_sq_list,rho,n_ul,nbrOfRealizations);
    error_val = @(s) mean(exp(-max(0, i_s(s) - log(2^b-1)))); % Exact RCUs
    [avg_error_ul(j),  s_val_ul(j)] = searchForCandidateS(error_val,1e-10,1); % Optimize s for the RCUs 
    ...
    i_s = @(s) info_density(s,g_list_dl,ghat_dl_list,sigma_sq_list_dl,rho,n_dl,nbrOfRealizations);
    error_val = @(s) mean(exp(-max(0, i_s(s) - log(2^b-1)))); % Exact RCUs
    [avg_error_dl(j),  s_val_dl(j)] = searchForCandidateS(error_val,1e-10,1); %Optimization over s    
...
filename = ['RCUs_exact_ULA_Sweep_Angle_' filename];
...
function [i_s] = info_density(s,g,ghat,sigma_list,rho,n,N)
% Function to obtain the generalized information density used to compute
% the exact RCUs.
i_s = 0;

g = repmat(g,N,1);
ghat = repmat(ghat,N,1);
ghat = ghat(:).';

for nn = 1:n
    z = sqrt(0.5)*sqrt(repmat(sigma_list,N,1)).*(randn(N,N)+1i*randn(N,N));
    x1 = sqrt(0.5)*sqrt(rho)*(randn(N,N)+1i*randn(N,N));
    ...
    i_s = i_s -s*abs(v-ghat.*q).^2 + s*abs(v).^2./(1+s*rho*abs(ghat).^2) + log(1+s*rho*abs(ghat).^2);
end
end
